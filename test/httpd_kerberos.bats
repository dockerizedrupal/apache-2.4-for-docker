...
@test "php: freetds: KERBEROS_1_SERVER_ADMIN" {
  run docker exec "$(container)" /bin/su - root -mc "cat /etc/krb5.conf | grep 'server_admin-1'"
...
@test "php: freetds: KERBEROS_2_SERVER_ADMIN" {
  run docker exec "$(container)" /bin/su - root -mc "cat /etc/krb5.conf | grep 'server_admin-2'"
...
@test "php: freetds: KERBEROS_3_SERVER_ADMIN" {
  run docker exec "$(container)" /bin/su - root -mc "cat /etc/krb5.conf | grep 'server_admin-3'"
